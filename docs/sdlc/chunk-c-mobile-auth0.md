...
5. Optional: tap **Fetch room (GET)** while the room is still `unpaid`. Expect **402 Entitlement unpaid** in the API error row — confirms the read gate without crashing the app.
6. Tap **Mark entitlement paid (staging)**. The card updates to `paid` (same token as `POST /race-rooms/:id/entitlement`).
