...
- **Branch:** `cursor/missing-test-coverage-c0b4` (coverage PR pending) from `main` @ `e0d1578`.
- **Active follow-up:** Review/merge coverage PR; staging deploy + signed-in chat smoke remain.

## Completed

- Coverage automation: added API regression coverage proving chat retention purges only eligible-room notification prefs, reports the purge count, and preserves prefs for ineligible rooms.
...
1. Review/merge the coverage PR after GitHub checks pass.
...
```text
Coverage PR on cursor/missing-test-coverage-c0b4 adds chat retention preference-purge tests; verify checks and merge. Then deploy staging (confirm 0014), reload mobile from main, and smoke chat send/photo/read receipt/load-older scroll.
```
