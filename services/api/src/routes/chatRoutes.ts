...
export async function chatRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/chat/rooms/:roomId/key-envelopes", async (request, reply) => {
    ...
    const requesterId = request.identity.sub;
    const { roomId } = request.params as { roomId: string };
    const room = await getRaceRoom(roomId);
    if (!room || !room.memberships.some((m) => m.userId === requesterId)) {
      return reply.code(403).send({ error: "Not a member of this room" });
    }
    ...
    if (keyVersion > latestKeyVersion + 1) {
      return reply.code(409).send({ error: "Envelope key version is out of sync" });
    }
