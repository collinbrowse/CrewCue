...
test("stale GET /mine snapshot does not wipe a newer stop-plan on the next write", async () => {
  const app = buildApp();
  await app.ready();
  const ownerToken = app.jwt.sign(buildClaims("owner-list-clobber"));
  const roomId = await createPaidRoom(app, ownerToken, "list cache clobber");
  await put50kCourse(app, roomId, ownerToken);
  ...
  const staleListSnapshot: RaceRoom = { ...roomAfterFirst };
